...
// ============ Decorative Elements ============
function Squiggle({ color = "var(--pink)", width = 80, height = 16 }) {
  return (
...
function Starburst({ size = 80, color = "var(--yellow)", children, rotate = -8 }) {
  return (
    <div style={{
      position: "relative",
...
function Triangle({ size = 30, color = "var(--cyan)", rotate = 0, style }) {
  return (
...
function Dot({ size = 14, color = "var(--pink)", style }) {
  return (
    <div style={{
      width: size, height: size,
      borderRadius: "50%",
...
function ZigZag({ width = 100, color = "var(--purple)" }) {
  return (
...
function ConfettiBg({ density = 1, opacity = 1 }) {
  const items = useMemo(() => {
...
// ============ Brand mark ============
// Uses the uploaded YGM calendar illustration
function BrandMark({ size = 72 }) {
  return (
    <img
      src="assets/ygm-logo.png"
...
// ============ Top Nav ============
function TopNav({ route, setRoute, signedIn }) {
  const links = signedIn
    ? [
      { id: "dashboard", label: "Dashboard" },
      { id: "designer", label: "Designer" },
      { id: "preview", label: "My Page" },
    ]
    : [
  ...
  return (
    <div className="top-nav">
      <a className="brand" onClick={() => setRoute("landing")}>
        <BrandMark />
        CLAUDENDLY
      </a>
      <div className="nav-links">
        ...
        {signedIn ? (
          <span style={{
            ...
            padding: "6px 12px", border: "2px solid var(--cream)",
          }}>LEE @ DOVO</span>
        ) : (
          <>
            <a onClick={() => setRoute("login")}>Log in</a>
            <button className="btn btn-pink" style={{ padding: "8px 14px", fontSize: 12 }}
...
Object.assign(window, { Squiggle, Starburst, Triangle, Dot, ZigZag, ConfettiBg, BrandMark, TopNav });
